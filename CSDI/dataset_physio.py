...
def parse_id_sales(matriz, missing_ratio = 0.1):
    observed_values = matriz
    observed_values = np.array(observed_values)
    observed_masks = ~np.isnan(observed_values)
...
def parse_store_sales():
    data = pd.read_csv('/content/drive/MyDrive/ColabNotebooks/train.csv', parse_dates=['date'])
    
    store_sales = data.groupby(['date', 'store_nbr'], as_index=False).aggregate({'sales': 'sum'})
    pivotada = store_sales.pivot(index = 'date', columns = 'store_nbr', values= 'sales')
    pivotada.replace(0, np.nan, inplace=True)
    matrices = []
    anios = pivotada.index.year.unique()
    meses = pivotada.index.month.unique()
    for i in anios:
        anio_i = pivotada[pivotada.index.year == i]
        for j in meses:
            matrices.append(np.array(anio_i[anio_i.index.month == j].iloc[:28].reset_index(drop = True)))
    return matrices[:54]


class Physio_Dataset1(Dataset):
...
class Physio_Dataset(Dataset):
    def __init__(self, eval_length=48, use_index_list=None, missing_ratio=0.0, seed=0):
        ...
        self.gt_masks = []
        path = (
            "./data/physio_missing" + "_mcar60_test" + ".pk"
        )
        #print(path)
        path_data = '/content/drive/MyDrive/ColabNotebooks/TFM/generated_data/mcar60'
        print('Data loaded from {}'.format(path_data))
        train = np.load(path_data+'/train.npy')
        print('train loaded', train.shape)
        val = np.load(path_data+'/val_x_hat.npy')
        print('val loaded' ,val.shape)
        test = np.load(path_data+'/test_x.npy')
        print('test_loaded', test.shape)
        
        #matrices = np.concatenate((train, val), axis=0)
        matrices = test
        
        print('Matrices cargadas', matrices.shape)
        if os.path.isfile(path) == False:  # if datasetfile is none, create
            print('not file')
            for matriz in matrices:
                try:
                    Qobserved_values, Qobserved_masks_, Qgt_masks_ = parse_id_sales(
                        matriz, missing_ratio
                    )
                    #print('matriz cargada')
                    #print(type(self.observed_values))
                    self.observed_values.append(Qobserved_values)
                    self.observed_masks.append(Qobserved_masks_)
                    self.gt_masks.append(Qgt_masks_)
                    #print(2)
                except Exception as e:
                    print('error', e)
                    #print(e)
