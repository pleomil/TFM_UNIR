...
def main(argv):
    ...
    t0 = time.time()
    with summary_writer.as_default(), tf.summary.should_record_summaries():
        for i, (x_seq, m_seq) in enumerate(tf_x_train_miss.take(num_steps)):
            try:
                ...
                if i % FLAGS.print_interval == 0:
                    ...
                    tf.summary.scalar("loss_val", val_loss)
                    tf.summary.scalar("kl_val", val_kl)
                    tf.summary.scalar("nll_val", val_nll)
